...
// Lớp FPSLimiter được định nghĩa ngay trong background script
class FPSLimiter {
    ...
    install() {
      if (this.config.enabled) {
        window.requestAnimationFrame = this.throttledRAF.bind(this);
      } else {
        this.uninstall();
      }
    }
...
async function manageScriptingForTab(tabId, url) {
    ...
    if (settingsCache.globalDisabled) {
        // Nếu bị tắt, xóa script khỏi trang nếu có
    ...
    // Tiêm script với cấu hình ban đầu
    chrome.scripting.executeScript({
        target: { tabId: tabId, allFrames: settingsCache.applyInFrames },
        func: (cfg) => {
            // Dán toàn bộ Class FPSLimiter vào đây để nó được định nghĩa trong trang
            // (Nội dung của class FPSLimiter ở trên)
            if (!window.myFpsLimiter) {
                window.myFpsLimiter = new FPSLimiter(cfg);
            } else {
                window.myFpsLimiter.update(cfg);
            }
        },
        args: [config],
        world: 'MAIN',
